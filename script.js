...
// Array of video URLs
const videoUrls = [
    'https://www.youtube.com/embed/UfBytG65dow?si=_hUuKwrLGjI1MdOD',
...
MusicModeButton.addEventListener('click', function() {
    const MusicModeText = 'Now is playing the classical theme music, you can choose other themes if you want';
    backgroundAudio.src = 'assets/musics/full_cd/chopin_nocturnes_cd1_pollini/audio.wav';
    backgroundAudio.onloadeddata = function() {
        playAudio();
    };
    console.log('Music mode button clicked, playing audio:', backgroundAudio.src);
    typeText(MusicModeText);
    MusicModeButton.textContent = 'You Are Now in Classical Mode';
    VideoModeButton.textContent = 'Swich to VidSeo Mode'; // Reset the other button's text
    ...
    body.classList.add('music-mode-active'); // Add class to body for classical mode
    body.classList.add('hide-dropdown'); // Add class to hide dropdown content
    hideExploreButton(); // Hide the explore button
    videoContainer.style.display = 'none'; // Hide video container
    legendVideo.src = ''; // Clear video source
    document.querySelector('h1').style.display = 'none';
    document.querySelectorAll('p').forEach(p => p.style.display = 'none');  // Optionally hide the first image element if necessary
    document.getElementById('videoToggleButton').classList.remove('pulse'); // Remove pulse animation
    ...
    exitMusicModeButton.style.display = 'block';
    VideoModeButton.style.display = 'none'; // Hide legend mode exit button
    copyrightContainer.style.display = 'none'; // Hide copyright container
});
...
typingText.addEventListener('mouseover', function(event) {
...
typingText.addEventListener('keydown', function(event) {
    if (event.key === 'Enter') {
        ...
        if (inputText === 'quick tour') {
            ...
            typeText("Command not recognized. Type 'More' to see the available commands.");
        }
    }
});
...
function toggleVideo() {
    ...
    if (videoToggleButton.textContent === 'Play the Video') {
        ...
        if (currentVideoIndex < videoUrls.length) {
            ...
            currentVideoIndex = 0;
            exitVideoModeButton.click(); // Exit legend mode when all videos are finished
        }
    }
}
